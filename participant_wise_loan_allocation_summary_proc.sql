DROP PROCEDURE IF EXISTS participant_wise_loan_allocation_summary_proc;
DELIMITER ;;
CREATE PROCEDURE participant_wise_loan_allocation_summary_proc(
...
DECLARE p_value int(11); 					-- To store individual participant IDs extracted from the input string
DECLARE done INT DEFAULT 0; 				-- Loop control variable
DECLARE separator_position INT; 			-- Position of the next comma in the participant ID string
...
DECLARE v_procedure_name VARCHAR(255) DEFAULT 'participant_wise_loan_allocation_summary_proc';
...
-- Create a temporary table for handling the closure filter based on the p_is_closure parameter
...
*/
  -- Step 2: interest_rate
	...
	-- Create a temporary table to calculate the interest rate for loan allocations
...
-- Step 2 interest_rate
 ...
 -- Step 3: temp_participant_ids
    SET v_step_name = 'Step 2: temp_participant_ids';
    SET v_start_time = NOW(6);
    INSERT INTO procedure_execution_log (procedure_name,step_name, start_time) VALUES (v_procedure_name,v_step_name, v_start_time);
	
-- Create a temporary table to store participant IDs extracted from the input string
...
WHILE NOT done DO
        SET separator_position = LOCATE(',', p_participant_id);

        IF separator_position > 0 THEN
                SET p_value = SUBSTRING(p_participant_id, 1, separator_position - 1);
                SET p_participant_id= SUBSTRING(p_participant_id, separator_position + 1);
        ELSE
                SET p_value = p_participant_id;
                SET done = 1;
        END IF;

        INSERT INTO temp_participant_ids (participant_id) VALUES (p_value);
END WHILE;
...
-- Step 3 temp_participant_ids
 ...
 -- Step 4: temp_allocation_summary_details
    SET v_step_name = 'Step 3: temp_allocation_summary_details';
    SET v_start_time = NOW(6);
    INSERT INTO procedure_execution_log (procedure_name,step_name, start_time) VALUES (v_procedure_name,v_step_name, v_start_time);
	
-- Create a temporary table to store loan allocation summary details
CREATE TEMPORARY TABLE temp_allocation_summary_details
SELECT *
FROM
 (SELECT pl.id,
	  pp.id as participation_party_id,
          pl.loan_name,
 ...
 (p_loanCollaborationType = 'syndication'
  AND pls.loan_collaboration NOT IN ('syndication',
   ...
   UNION SELECT pla.id,
                ...
                coalesce(pl_sum.outstanding_balance,0) as outstanding_loan_bal,
                coalesce(pl_sum.principal_paid,0) as principal_paid,
   ...
   /*  AND EXISTS
       (SELECT 1
        FROM temp_is_closure tc
        WHERE tc.is_closure=pla.is_closure)*/
) grp
JOIN temp_participant_ids tt ON tt.participant_id = grp.participation_party_id
...
ORDER BY correlation_column,
		...
		-- Create an index on the temporary table for performance improvement
