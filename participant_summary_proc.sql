...
DROP TEMPORARY TABLE IF EXISTS temp_participant_role;
 ...
 -- Step 1: Initialize Step Logging
    SET v_procedure_name ='participant_summary_proc';
...
LEFT JOIN
  (SELECT participant_party_id,
   ...
   GROUP BY participant_party_id) nsa ON nsa.participant_party_id = pp.id
...
-- Step 5: temp_participation_user_details
	...
	WITH all_updates AS (
	    -- Combine updates from all related tables
	    SELECT id AS participant_party_id, updated_by, updated_at FROM participant_parties
	    UNION ALL
	    SELECT participant_party_id, updated_by, updated_at FROM participant_client_bank_accounts
	    UNION ALL
	    SELECT participant_party_id, updated_by, updated_at FROM participant_mail_addresses
	    UNION ALL
	    SELECT participant_party_id, updated_by, updated_at FROM participant_contacts
			),
	ranked_updates AS (
	    -- Assign ranks to updates per participant
		    SELECT DISTINCT au.participant_party_id, au.updated_by, au.updated_at,
		           RANK() OVER (PARTITION BY au.participant_party_id ORDER BY au.updated_at DESC) AS rnk
		      FROM all_updates au
		      JOIN temp_participation_details pp ON pp.id = au.participant_party_id
			  )

		    SELECT DISTINCT ru.participant_party_id, bs.user_name AS updated_by, ru.updated_at
		     FROM ranked_updates ru
		     LEFT JOIN base_users bs ON ru.updated_by = bs.id
		   WHERE ru.rnk = 1;*/
	  ...
	  (SELECT participant_party_id,
	   ...
	   FROM
		 (SELECT pp.id participant_party_id,
				 GREATEST(COALESCE( -- Selects the most recent update from multiple sources.
								   (SELECT MAX(CONCAT(COALESCE(ppt.updated_at, '1970-00-00 00:00:00'), ';', COALESCE(ppt.updated_by, 0)))
